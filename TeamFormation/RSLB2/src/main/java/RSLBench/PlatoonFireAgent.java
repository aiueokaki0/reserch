        ...
        if(listMsg != null && !listMsg.isEmpty()) {
	        for(Message msg : listMsg) {
	        	if(msg != null) {
		        	...
		        	if(str.substring(0, 9).equals("CIVILIAN:"))
		        	{
		        		str = str.substring(9);
		        		if(model.getEntity(assignedTarget) instanceof Human && ((Human)(model.getEntity(assignedTarget))).getPosition(model) instanceof Building) {
		        			assignedTarget = new EntityID(Integer.valueOf(str));
		        			Logger.info("HELPING FIRE AGENT");
		        		}
		        	}
	        	}
	        }
