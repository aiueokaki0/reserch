...
public final class Launcher {
        ...
        try {
            ...
            for (int i = 0; i < args.length; ++i) {
                if (args[i].equals(FIRE_BRIGADE_FLAG)) {
                    ...
                    Logger.info(Markers.GREEN, "fb="+fb);
                }
                else if (args[i].equals(POLICE_FORCE_FLAG)) {
                    pf = Integer.parseInt(args[++i]);
                    Logger.info(Markers.GREEN, "pf="+pf);
                }
                else if (args[i].equals(AMBULANCE_TEAM_FLAG)) {
                    at = Integer.parseInt(args[++i]);
                    Logger.info(Markers.GREEN, "at="+at);
                }
                else if (args[i].equals(CIVILIAN_FLAG)) {
                    cv = Integer.parseInt(args[++i]);
                    Logger.info(Markers.GREEN, "cv="+cv);
                }
                else {
            ...
            connect(launcher, fb, pf, at, cv, config);
        }
     ...
     */
    private static void connect(ComponentLauncher launcher, int fb, int pf, int at, int cv, Config config) throws InterruptedException, ConnectionException {
        ...
        List<CivilianAgent> civilianAgents = new ArrayList<>();
        ...
        try {
            while (cv-- != 0) {
                ...
                CivilianAgent civilianAgent = new CivilianAgent();
        ...
        try {
            ...
            launcher.connect(new CenterAgent(fireAgents, policeAgents, ambulanceAgents, civilianAgents));
            Logger.info("success");
        }
        catch (ComponentConnectionException e) {
            Logger.info("failed: " + e.getMessage());
