...
public class ProblemDefinition {
    ...
    private ArrayList<EntityID> ambulanceAgents;
    private ArrayList<EntityID> fires;
    private ArrayList<EntityID> blockades;
    private ArrayList<EntityID> civilians;
    ...
    private double[][] ambulanceUtilityMatrix;
    ...
    public ProblemDefinition(Config config, ArrayList<EntityID> fireAgents,
            ...
            StandardWorldModel world, ArrayList<EntityID> ambulanceAgents,
            ArrayList<EntityID> civilians) {
        ...
        buildAmbulanceUtilityMatrix(lastAssignment);
    ...
    private void buildAmbulanceUtilityMatrix(Assignment lastAssignment) {
        final int nAgents = ambulanceAgents.size();
        final int nTargets = civilians.size();
        ambulanceUtilityMatrix = new double[nAgents][nTargets];
        for (int i=0; i<nAgents; i++) {
            final EntityID agent = ambulanceAgents.get(i);
            id2idx.put(agent, i);

            for (int j=0; j<nTargets; j++) {
                final EntityID target = civilians.get(j);
                if (i == 0) {
                    id2idx.put(target, j);
                }

                double utility = utilityFunction.getAmbulanceUtility(agent, target);
                ...
                ambulanceUtilityMatrix[i][j] = utility;
            }
        }
    }
    ...
    private HashMap<Pair<EntityID, EntityID>, EntityID> blockedAmbulanceAgents = new HashMap<>();
    ...
    private void computeBlockedAmbulanceAgents() {
        Logger.debug("Computing blocked ambulance agents...");
        for (EntityID agent : getAmbulanceAgents()) {
            Human hagent = (Human)world.getEntity(agent);
            EntityID position = hagent.getPosition();

            for (EntityID target: getCivilians()) {
                SearchResults results = pathDB.search(position, ((Human)world.getEntity(target)).getPosition());
                List<Blockade> pathBlockades = results.getPathBlocks();
                if (!pathBlockades.isEmpty()) {
                    Logger.trace("Ambulance {} blocked from reaching civilian {} by {}", agent, target, pathBlockades.get(0).getID());
                    blockedAmbulanceAgents.put(new Pair<>(agent, target), pathBlockades.get(0).getID());
                }
            }
        }
        Logger.debug("Done computing blocked ambulance agents.");
    }
    ...
    public double getFireUtility(EntityID firefighter, EntityID fire) {
    	if(id2idx.get(firefighter) != null && id2idx.get(fire) != null) {
	        final int i = id2idx.get(firefighter);
    ...
    public double getPoliceUtility(EntityID police, EntityID blockade) {
    	if(id2idx.get(police) != null && id2idx.get(blockade) != null) {
    ...
    public double getAmbulanceUtility(EntityID ambulance, EntityID civilian) {
    	if(id2idx.get(ambulance) != null && id2idx.get(civilian) != null) {
	        final int i = id2idx.get(ambulance);
	        final int j = id2idx.get(civilian);
	        return ambulanceUtilityMatrix[i][j];
    	}
    	else
    		return Double.MIN_VALUE;
    }
     ...
     * Returns the target with the highest utility for the agent
     *
     * @param ambulanceAgent the agentID
     * @return the targetID
     */
    public EntityID getHighestTargetForAmbulanceAgent(EntityID ambulanceAgent) {
        return getHighestTargetForFireAgent(ambulanceAgent, civilians);
    }
     ...
     * @return the targetID
     */
    public EntityID getHighestTargetForAmbulanceAgent(EntityID ambulanceAgent, List<EntityID> candidateCivilians) {
        double best = -Double.MAX_VALUE;
        EntityID civ = candidateCivilians.get(0);
        for (EntityID t : candidateCivilians) {
            final double util = getAmbulanceUtility(ambulanceAgent, t);
            if (util > best) {
                best = util;
                civ = t;
            }
    ...
    /**
     * Returns a list of civilians in the problem.
     * @return list of civilians.
     */
    public ArrayList<EntityID> getCivilians() {
        return civilians;
    }
    ...
    /**
     * Returns the ambulance agents.
     * @return the ambulance agents.
     */
    public ArrayList<EntityID> getAmbulanceAgents() {
