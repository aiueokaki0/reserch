...
public class PlatoonPoliceAgent extends PlatoonAbstractAgent<PoliceForce>
{
    ...
    public ArrayList<EntityID> listHelp = new ArrayList<EntityID>();
    
    /** EntityID of the road where the blockade that this agent should remove is located */
    public EntityID assignedTarget = Assignment.UNKNOWN_TARGET_ID;

    public PlatoonPoliceAgent() {
    	Logger.info(Markers.BLUE, "Platoon Police Agent CREATED");
    }
    ...
    protected void think(int time, ChangeSet changed, Collection<Command> heard) {
        ...
        listHelp = new ArrayList<EntityID>();
        
        List<Message> listMsg = centerAgent.comLayer.retrieveMessages(getID());
        
	    if(listMsg != null && !listMsg.isEmpty()) {
	        for(Message msg : listMsg) {
	        	if(msg != null) {
		        	String str = ((MessageString)msg).getMsg();
		        	if(str.substring(0, 11).equals("BARRICADES:"))
		        	{
		        		str = str.substring(11);
			        	for(String id : Arrays.asList(str.split(","))){
			        		if(id != "") {
			        			boolean add = true;
			        			for(EntityID existingid : listHelp) {
			        				if(existingid.getValue() == Integer.valueOf(id)) {
			        					add = false;
			        					break;
			        				}
			        			}
			        			if(add)
			        				listHelp.add(new EntityID(Integer.valueOf(id)));
			        		}
			        	}
		        	}
	        	}
        ...
        if (assignedTarget != null && !assignedTarget.equals(Assignment.UNKNOWN_TARGET_ID)) {
            ...
            if (target != null) {
                ...
                // Clear if in range
                if (inRange(target)) {
                    Logger.info(Markers.BLUE, "Police force {} clearing ASSIGNED target {}", getID(), assignedTarget);
        ...
        if (!config.getBooleanValue(Constants.KEY_AGENT_ONLY_ASSIGNED)) {
            ...
            if (inRange(target)) {
                Logger.info(Markers.BLUE, "Police force {} clearing self-assigned target {}", getID(), bestTarget);
    ...
    private boolean approach(int time, EntityID target) {
        ...
        if (!blocks.isEmpty() && inRange(blocks.get(0))) {
            ...
            Logger.info(Markers.MAGENTA, "Police force {} clearing blockade {} to reach ASSIGNED target {} through {}", getID(), block, target, path);
            sendClear(time, block.getID());
            return true;
        }

        Logger.info(Markers.MAGENTA, "Police force {} approaching ASSIGNED target {} through {}", getID(), target, path);
        sendMove(time, steps);
        return true;
    }
