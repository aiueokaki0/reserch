...
public class BreadthFirstSearch extends AbstractSearchAlgorithm
{
    ...
    /*
    public SearchResults searchWithBlockades(PlatoonAbstractAgent<StandardEntity> agent, Area start, Collection<Area> goals, Graph graph, DistanceInterface distanceMatrix)
    {
        List<Area> open = new LinkedList<>();
        Map<Area, Area> ancestors = new HashMap<>();
        open.add(start);
        ...
        agent.idBlocks = new ArrayList<EntityID>();
        for(Blockade block : blockers) {
        	agent.idBlocks.add(block.getID());
        }
        
        Logger.info(Markers.LIGHT_BLUE,agent.idBlocks);
