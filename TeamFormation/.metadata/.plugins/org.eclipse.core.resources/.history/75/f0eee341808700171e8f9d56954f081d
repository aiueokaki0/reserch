...
public class PlatoonAmbulanceAgent extends PlatoonAbstractAgent<AmbulanceTeam>
{
    ...
    protected void think(int time, ChangeSet changed, Collection<Command> heard) {
        ...
        if (!assignedTarget.equals(Assignment.UNKNOWN_TARGET_ID) && ((Human)(model.getEntity(assignedTarget))).getPosition(model) instanceof Building) {
        ...
        if(assignedTarget != Assignment.UNKNOWN_TARGET_ID && CivilianAgent.wounded.containsKey(assignedTarget)) {
	        if(CivilianAgent.wounded.get(assignedTarget)) {
		        String msg = "CIVILIAN:" + ((Human) model.getEntity(assignedTarget)).getPosition();
	        	broadcastMsg(msg);
	        }
        }
        // Start to act
        ...
        // //////////////////////////////////////
        if (this.withCivilian && !(location() instanceof Refuge)) {
        ...
        if (!assignedTarget.equals(Assignment.UNKNOWN_TARGET_ID) && !CivilianAgent.rescued.get(assignedTarget) && ((Human)(model.getEntity(assignedTarget))).getPosition(model) instanceof Area) {

            // Rescue if the assigned target is in range
            if (model.getDistance(me().getPosition(), assignedTarget) <= range) {
                Logger.info(Markers.MAGENTA, "Agent {} rescuing ASSIGNED target {}", getID(), assignedTarget);
                sendLoad(time, assignedTarget);
                withCivilian = true;
                return;
            }
