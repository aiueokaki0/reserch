...
public class CenterAgent extends StandardAgent<Building> {
    ...
    public ArrayList<EntityID> fireAgentsIDs = new ArrayList<>();
    public ArrayList<EntityID> policeAgentsIDs = new ArrayList<>();
    public ArrayList<EntityID> ambulanceAgentsIDs = new ArrayList<>();
    public ArrayList<EntityID> civilianAgentsIDs = new ArrayList<>();
    private Assignment lastAssignment = new Assignment();
    public static List<PlatoonFireAgent> fireAgents;
    public static List<PlatoonPoliceAgent> policeAgents;
    public static List<PlatoonAmbulanceAgent> ambulanceAgents;
    public static List<CivilianAgent> civilianAgents;
    private List<Blockade> blockades = new ArrayList<>();
    
    public CommunicationLayer comLayer = new CommunicationLayer();

    public CenterAgent(List<PlatoonFireAgent> fireAgents,
            List<PlatoonPoliceAgent> policeAgents, List<PlatoonAmbulanceAgent> ambulanceAgents, List<CivilianAgent> civilianAgents) {
        ...
        model.addWorldModelListener(new WorldModelListener<StandardEntity>() {
            ...
            public void entityRemoved(WorldModel<? extends StandardEntity> model,
                ...
                if (e instanceof Blockade) {
                    ...
                    for (CivilianAgent civilian : civilianAgents) {
                        civilian.removeBlockade(blockade);
                    }
                }
            }
        });
    ...
    protected void think(int time, ChangeSet changed, Collection<Command> heard) {
        ...
        Collection<EntityID> civs = getCivilians();
        Logger.info(Markers.WHITE, "TIME IS {} | {} burning buildings | {} blockades | {} civilians",
                new Object[]{time, burning.size(), blockades.size(), civs.size()});
        ...
        if (burning.isEmpty() && civs.isEmpty()) {// && blockades.isEmpty()) {
            Logger.info("All fires extinguished and all civilians rescued. Good job!");
            System.exit(0);
        }

        // Build the problem
        ArrayList<EntityID> fires = new ArrayList<>(burning);
        ArrayList<EntityID> civilians = new ArrayList<>(civs);
        ProblemDefinition problem = new ProblemDefinition(config, fireAgentsIDs,
                fires, policeAgentsIDs, blockadeIDs, lastAssignment, model, ambulanceAgentsIDs, civilians);
    ...
    public Collection<EntityID> getCivilians() {
        ...
        for (StandardEntity next : e) {
            if (next instanceof Civilian) {
                ...
                if (b.getHP() > 0 && !(b.getPosition(model) instanceof Refuge) && !(b.getPosition(model) instanceof AmbulanceCentre))
                	result.add(b);
            }
        }
        // Sort by distance
